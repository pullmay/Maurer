...
/* MTで生成した系列をAESで変換し，変換後の系列に対して検定を行う */
/* 最後にファイルを保存するので，ディレクトリを用意しておくこと */
/* 検定本数はarraysizeで指定 */

void flipping(int n, double p, char *s, unsigned char *key_exp, unsigned int *PT, int *unchi){
  unsigned int hogehoge[4];
  for (int i=0;i<n;i++){
    // if (s[i]==1 && genrand_res53()<=p){
    if(unchi[0]==4){
      unchi[0]=0;
      aes(key_exp,PT,hogehoge);
    if (PT[3]==pow(2,32)-1){
      PT[2] += 1;
      PT[3] = 0;
    } else {
    ...
    if (s[i]==1 && (double)hogehoge[unchi[0]]/pow(2,32)<=p){ 
      s[i]=0;
    }
    unchi[0]++;
  }
...
int main(){
  ...
  //1000回
    ...
    for (int k=0; k<16160; k++){
      ...
      ciphertext[0] = genrand_int32();
      ciphertext[1] = genrand_int32();
      ciphertext[2] = genrand_int32();
      ciphertext[3] = genrand_int32();
      //ビットに変換
      for(int i=0;i<4;i++){
        for(int j=31;j>=0;j--){
    ...
    //変換
    printf("now:%d\n",l);
    int cnt1=count(n,rand);
    printf("%d/%d=%f\n", cnt1, n, (double)cnt1/n);
    flipping(n, 1-2*q, rand, key_exp, PT, unchi);//-------------------------------flip
    int cnt2=count(n,rand);
    printf("%d/%d=%f\n", cnt2, n, (double)cnt2/n);
  ...
  // FILE *fp;
  // FILE *gp;
  // fp=fopen("./file/proposed_unchi2_1000","wb");
  // gp=fopen("./file/yamamoto_unchi2_1000","wb");
